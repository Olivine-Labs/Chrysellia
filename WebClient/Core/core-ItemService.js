...
(function( window, undefined ) {
	...
	ItemService = ItemService.prototype = {
		...
		GetInventory: function(callback){
			var data = { };
			vc.SendQueuedRequest(vc.TYPE_ITEM, vc.ch.ACTION_GETINVENTORY, data).success( function(data) { callback(data); } );
		},
	
		Equip: function(itemId, slotType, slot, callback){
			var data = { ItemId: itemId, SlotNumber: slot };
			vc.SendQueuedRequest(vc.TYPE_ITEM, vc.ch.ACTION_EQUIP, data).success( function(data) { callback(data, itemId, slotType, slot); } );
		},
		
		UnEquip: function(itemId, slotType, slot, callback){
			var data = { ItemId: itemId };
			vc.SendQueuedRequest(vc.TYPE_ITEM, vc.ch.ACTION_UNEQUIP, data).success( function(data) { callback(data, itemId, slotType, slot); } );
		},
		
		SendTrade: function(itemId, gold, playerName, callback){
			var data = { ItemId: itemId, Gold: gold, Player: playerName };
			vc.SendQueuedRequest(vc.TYPE_ITEM, vc.ch.ACTION_SEND_TRADE, data).success( function(data) { callback(data); } );
		},
		
		AcceptTrade: function(tradeId, callback){
			var data = { Trade: tradeId };
			vc.SendQueuedRequest(vc.TYPE_ITEM, vc.ch.ACTION_ACCEPT_TRADE, data).success( function(data) { callback(data); } );
		}
	}
