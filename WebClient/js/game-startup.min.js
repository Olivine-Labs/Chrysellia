...
b=$this.attr("class").split(" ")[1].replace(/itemType_/,"")*1;a=$("#itemsWindow select."+a).index($this);$this.val()==0?vc.is.UnEquip(window.MyCharacter.Equipment[b][a].ItemId,b,a,UnEquipItem):vc.is.Equip($this.val(),b,a,EquipItem)})});
function EquipItem(a,b,c,d){if(a.Result==vc.ER_SUCCESS){a={};var e=vc.is.TypeMapping,f;for(f in window.MyCharacter.Inventories.Personal)if(window.MyCharacter.Inventories.Personal[f].ItemId=b){window.MyCharacter.Equipment[c][d]=window.MyCharacter.Inventories.Personal[f];delete window.MyCharacter.Inventories.Personal[f];a=window.MyCharacter.Equipment[c][d];$("option[value="+a.ItemId+"]").remove();$("<option value='"+a.ItemId+"'>"+a.Name+"</option>").appendTo($("select."+e[a.SlotType]).eq(d))}}}
function UnEquipItem(a,b,c,d){if(a.Result==vc.ER_SUCCESS)window.MyCharacter.Equipment[c][d]={}}function RefreshMap(a){if(a.Result==vc.ER_SUCCESS){MyCharacter.PositionX=a.Data.X;MyCharacter.PositionY=a.Data.Y;BuildMap()}}function SetEnableMovement(a){$("#movementform button").button("option","disabled",!a).removeClass("ui-state-hover")}
