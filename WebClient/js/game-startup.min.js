...
_("chatForm").bind("submit",function(a){a.preventDefault();a=_("chatInput")[0];var b=a.value;if(b.length>500)b=b.substr(0,500);SubmitMessage(b);a.value=""});window.$tabs=$("#chatChannels").tabs({tabTemplate:'<li><a href="#{href}" class="chatChannelLabel">#{label}</a> <span class="ui-icon ui-icon-close">Remove Tab</span></li>',add:function(a,b){$(b.panel).append("<div />");$tabs.tabs("select","#"+b.panel.id)},select:function(a,b){if($(b.panel).children(".channelId").length>0){window.MyCharacter.CurrentChannel=
$(".channelId",b.panel)[0].value;$(b.tab).parent().removeClass("newMessage")}}});$("#chatChannels span.ui-icon-close").live("click",function(){var a=$(".channelId",$($(this).siblings("a")[0].getAttribute("href")))[0].value;a!=vc.ch.StaticRooms.General&&a!=vc.ch.StaticRooms.Trade&&LeaveChannel(a)});_("createChannelForm").dialog({autoOpen:false,title:"Create Chat Channel",width:400});_("joinChannelForm").dialog({autoOpen:false,title:"Join Chat Channel",width:400});$("#createChannelForm form").bind("submit",
function(a){a.preventDefault();a=_("cc_channelName")[0].value;var b=_("cc_channelMOTD")[0].value,d=_("cc_publicRead")[0].checked,e=_("cc_publicWrite")[0].checked;d=d?1:0;e=e?1:0;vc.ch.CreateChannel(a,b,d,e,CreateChannel)});$("#joinChannelForm form").bind("submit",function(a){a.preventDefault();a=_("jc_channelName")[0].value;vc.ch.JoinChannel(a,JoinChannel)});$("#quickLoginForm .button").bind("click",function(a){a.preventDefault();vc.as.Logout(Logout)});_("createChannelLink").bind("click",function(){_("createChannelForm").dialog("open")});
...
case "stat int":vc.cs.LevelUp(4,LevelUpResponse);break;case "stat wis":vc.cs.LevelUp(5,LevelUpResponse);break;case "stat all":vc.cs.LevelUp(0,LevelUpResponse)}});$(document).keydown(function(a){a.which==192&&OpenDebugWindow()})});
