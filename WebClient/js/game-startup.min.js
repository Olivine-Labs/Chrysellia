$(function(){vc.DebugMode=true;LoadICache();if($.cookie("theme")=="dark"){$("body").addClass("dark");_("themeSelect").children("options").eq(1).attr("selected","selected")}_("themeSelect").bind("change",function(){$this=$(this);if($this[0].value==0){$("body").removeClass("dark");$.cookie("theme","light")}else{$("body").addClass("dark");$.cookie("theme","dark")}});window.chatTabIndex=0;ICache.myCharacter_ExperienceBar.progressbar();ICache.myCharacter_HealthBar.progressbar();vc.cs.GetCurrentCharacter(SelectCharacter);
_("chatForm").bind("submit",function(a){a.preventDefault();a=_("chatInput")[0];var b=a.value;if(b.length>500)b=b.substr(0,500);SubmitMessage(b);a.value=""});window.$tabs=$("#chatChannels").tabs({tabTemplate:'<li><a href="#{href}" class="chatChannelLabel">#{label}</a> <span class="ui-icon ui-icon-close">Remove Tab</span></li>',add:function(a,b){$(b.panel).append("<div />");$tabs.tabs("select","#"+b.panel.id)},select:function(a,b){window.MyCharacter.CurrentChannel=$(".channelId")[0].value;$(b.tab).parent().removeClass("newMessage")}});
$("#chatChannels span.ui-icon-close").live("click",function(){var a=$(".channelId",$($(this).siblings("a")[0].getAttribute("href")))[0].value;a!=vc.ch.StaticRooms.General&&a!=vc.ch.StaticRooms.Trade&&LeaveChannel(a)});_("createChannelForm").dialog({autoOpen:false,title:"Create Chat Channel",width:400});_("joinChannelForm").dialog({autoOpen:false,title:"Join Chat Channel",width:400});$("#createChannelForm form").bind("submit",function(a){a.preventDefault();a=_("cc_channelName")[0].value;var b=_("cc_channelMOTD")[0].value,
d=_("cc_publicRead")[0].checked,e=_("cc_publicWrite")[0].checked;d=d?1:0;e=e?1:0;vc.ch.CreateChannel(a,b,d,e,CreateChannel)});$("#joinChannelForm form").bind("submit",function(a){a.preventDefault();a=_("jc_channelName")[0].value;vc.ch.JoinChannel(a,JoinChannel)});$("#quickLoginForm .button").bind("click",function(a){a.preventDefault();vc.as.Logout(Logout)});_("createChannelLink").bind("click",function(){_("createChannelForm").dialog("open")});_("joinChannelLink").bind("click",function(){_("joinChannelForm").dialog("open")});
_("moveNW").button({icons:{primary:"ui-icon-arrowthick-1-nw"},text:false});_("moveN").button({icons:{primary:"ui-icon-arrowthick-1-n"},text:false});_("moveNE").button({icons:{primary:"ui-icon-arrowthick-1-ne"},text:false});_("moveW").button({icons:{primary:"ui-icon-arrowthick-1-w"},text:false});_("moveE").button({icons:{primary:"ui-icon-arrowthick-1-e"},text:false});_("moveSW").button({icons:{primary:"ui-icon-arrowthick-1-sw"},text:false});_("moveS").button({icons:{primary:"ui-icon-arrowthick-1-s"},
text:false});_("moveSE").button({icons:{primary:"ui-icon-arrowthick-1-se"},text:false});$("#movementform button").bind("click",function(a){a.preventDefault();SetEnableMovement(false);$this=$(this);a=$this.siblings(".x")[0].value*1;var b=$this.siblings(".y")[0].value*1;Move(a,b)});_("statsWindow").dialog({title:"Character Stats",autoOpen:false});_("itemsWindow").dialog({title:"Inventory",autoOpen:false});$(".accountActions .logOut").bind("click",function(a){a.preventDefault();vc.as.Logout(Logout)});
_("statsWindowButton").bind("click",function(a){a.preventDefault();_("statsWindow").dialog("open")});$(".chooseStats").live("click",function(a){a.preventDefault();_("statsWindow").dialog("open")});_("itemsWindowButton").bind("click",function(a){a.preventDefault();_("itemsWindow").dialog("open")});$(".chatMessage.system a.joinChannel").live("click",function(a){a.preventDefault();$this=$(this);a=$this.siblings(".channelName").children("input:eq(0)")[0].value;vc.ch.JoinChannel(a,JoinChannel);$this.parentsUntil(".ui-dialog").parent().dialog("destroy").remove()});
$("#itemsWindow select").live("change",function(){$this=$(this);var a=$this[0].className.split(" ")[0];$("#itemsWindow select").attr("disabled","disabled");var b=$this[0].className.split(" ")[1].replace(/itemType_/,"")*1,d=$("#itemsWindow select."+a).index($this);if($this[0].value==0)vc.is.UnEquip(window.MyCharacter.Equipment[b][d].ItemId,b,d,UnEquipItem);else MyCharacter.Equipment[b][d]!=={}?vc.is.UnEquip(window.MyCharacter.Equipment[b][d].ItemId,b,d,function(){vc.is.Equip($this[0].value,b,d,EquipItem)}):
vc.is.Equip($this[0].value,b,d,EquipItem)});$(window).resize(function(){ResizeChat()});$("#statsWindow button").bind("click",function(a){a.preventDefault();$(".chooseStats").remove();switch($(this).parent()[0].className){case "stat str":vc.cs.LevelUp(1,LevelUpResponse);break;case "stat dex":vc.cs.LevelUp(2,LevelUpResponse);break;case "stat vit":vc.cs.LevelUp(3,LevelUpResponse);break;case "stat int":vc.cs.LevelUp(4,LevelUpResponse);break;case "stat wis":vc.cs.LevelUp(5,LevelUpResponse);break;case "stat all":vc.cs.LevelUp(0,
LevelUpResponse)}});$(document).keydown(function(a){a.which==192&&OpenDebugWindow()})});
function Move(a,b){SetEnableMovement(false);var d=a+MyCharacter.PositionX*1,e=b+MyCharacter.PositionY*1;if(d>MyCharacter.CurrentMap.DimensionX-1||MyCharacter.PositionX<0)d=MyCharacter.PositionX;if(e>MyCharacter.CurrentMap.DimensionY-1||e<0)e=MyCharacter.PositionY;vc.ms.Move(d,e,RefreshMap);a+b==1||a+b==-1?window.setTimeout(function(){SetEnableMovement(true)},200):window.setTimeout(function(){SetEnableMovement(true)},282)}
...
function LevelUpResponse(a,b){if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}var d=b.Stat;if(a.Result==vc.ER_SUCCESS){switch(d){case 0:MyCharacter.Strength+=12;MyCharacter.Dexterity+=12;MyCharacter.Vitality+=12;MyCharacter.Intelligence+=12;MyCharacter.Wisdom+=12;break;case 1:MyCharacter.Strength+=50;break;case 2:MyCharacter.Dexterity+=50;break;case 3:MyCharacter.Vitality+=50;break;case 4:MyCharacter.Intelligence+=
50;break;case 5:MyCharacter.Wisdom+=50}MyCharacter.Strength+=MyCharacter.RacialStrength;MyCharacter.Dexterity+=MyCharacter.RacialDexterity;MyCharacter.Vitality+=MyCharacter.RacialVitality;MyCharacter.Intelligence+=MyCharacter.RacialIntelligence;MyCharacter.Wisdom+=MyCharacter.RacialWisdom;MyCharacter.Level++;MyCharacter.FreeLevels--;MyCharacter.FreeLevels<1&&$("#statsWindow button, #statsWindow .all").hide();MyCharacter.Health=MyCharacter.Vitality;UpdateStats()}Log("Level Up: "+JSON.stringify(b))}
function ResizeChat(){$("#chatChannels .ui-tabs-panel").css({height:$(window).height()-432+"px"})}
...
window.MyCharacter.Equipment[d][e];$("option[value='"+g.ItemId+"']").remove();$("<option value='"+g.ItemId+"' selected='selected'>"+g.Name+"</option>").appendTo($("select."+h[g.SlotType]).eq(e))}Log("Equip Item: "+JSON.stringify(b));$("#itemsWindow select").removeAttr("disabled")}
...
f;window.MyCharacter.Equipment[d][e]={}}$("#itemsWindow select").removeAttr("disabled");MyCharacter.CurrentMap.Places[MyCharacter.PositionX][MyCharacter.PositionY].PlaceType==vc.ms.PLACE_TYPE_STORE&&BuildGameWindow()}
function RefreshMap(a,b){if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS){MyCharacter.PositionX=a.Data.X;MyCharacter.PositionY=a.Data.Y;BuildMap(false)}Log("Refresh Map: "+JSON.stringify(b))}function SetEnableMovement(a){$("#movementform button").button("option","disabled",!a).removeClass("ui-state-hover")}
...
function FillChat(a){if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS)for(var b in a.Data)if(b!=0)InsertChat(a.Data[b],b);else typeof a.Data[0]=="object"&&a.Data[0].length>0&&ProcessSystemMessage(a.Data[0]);$(".chatMessage:nth-child(n+50)").remove();window.setTimeout(function(){vc.ch.GetMessagesFromChannel(MyCharacter.CurrentChannel,FillChat)},4500)}
function SelectCharacter(a,b){window.MyCharacter=new Character;Log("Login: "+a.Result);if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS){window.MyCharacter.Construct(a.Data);UpdateStats()}else if(!vc.DebugMode){alert("Please login again.");window.location="./index.php"}for(var d in window.MyCharacter.Channels)d!="remove"&&AddTab(window.MyCharacter.Channels[d].Name,d,
window.MyCharacter.Channels[d].Motd);Log("Select Character: "+JSON.stringify(b));$tabs.tabs("select",0);vc.ch.GetMessagesFromChannel(d,FillChat);window.ChatQueue=new Queue({Timeout:4500});window.ChatQueue.AddItem(vc.TYPE_CHAT,vc.ch.ACTION_GETMESSAGESFROMCHANNEL,{Channel:window.MyCharacter.CurrentChannel},FillChat);vc.is.GetInventory(LoadInventory)}
function BuildMap(a){$(ICache.currentMapName).text(window.MyCharacter.CurrentMap.Name);$(ICache.currentMapPosition).text(window.MyCharacter.PositionX+" , "+MyCharacter.PositionY);if(a)$.getScript("./core/staticInfo/"+window.MyCharacter.CurrentMap.Name+".js",function(){BuildMapTable();BuildGameWindow()});else{BuildMapTable();BuildGameWindow()}}
function BuildMapTable(){var a=$(ICache.currentMap),b=20*-window.MyCharacter.PositionX+65,d=20*(window.MyCharacter.PositionY-MyCharacter.CurrentMap.Places[MyCharacter.PositionX].length)+85;console.log(b+", "+d);a.css("background",'url("./css/images/large_maps/'+window.MyCharacter.CurrentMap.Name+'_20.png") no-repeat scroll '+b+"px "+d+"px transparent")}
function BuildGameWindow(){var a=MyCharacter.CurrentMap.Places[MyCharacter.PositionX][MyCharacter.PositionY],b=MyCharacter.CurrentMap.Monsters[a.LocationType],d=_("topCenter");d.html("&nbsp;");if(a.PlaceType===undefined){if(b!==undefined){a=$("<form id='fightForm'><h1>Fight!</h1><span class='attackLabel'>Attack:</span></form>");var e=$("<select id='monsterList' />");e.appendTo(a);var f=["lowLevel","averageLevel","highLevel"],g=f[1];$("<button type='submit' id='attackButton' class='button attack'>Attack</button>").bind("click",
...
V2Core.Monsters[b[m]];if(h.Level-3<=0.75*MyCharacter.Level)g=f[0];else if(h.Level-3>=1.25*MyCharacter.Level)g=f[2];$("<option value='"+h.Id+"' class='"+g+"'>"+h.Name+"</option>").appendTo(j)}j.appendTo(e);a.appendTo(d);$("<div id='fightResults' class='fightResults'></div>").appendTo(d)}}else switch(a.PlaceType){case vc.ms.PLACE_TYPE_STORE:BuildShop(d);break;case vc.ms.PLACE_TYPE_SHRINE:BuildShrine(d);break;case vc.ms.PLACE_TYPE_BANK:BuildBank(d);break;case 3:BuildExit(d)}}
function SubmitBankTransaction(){var a=_("transactionAmount")[0].value,b=_("transactionTypeSelection")[0].value;if(b==0&&a=="all"||b==2){a=MyCharacter.Gold*1;b=0}else if(b==1&&a=="all"||b==3){a=MyCharacter.Bank*1;b=1}b*=1;if(isInteger(a)||a>0)switch(b){case 0:if(a>MyCharacter.Gold)a=MyCharacter.Gold;vc.ms.Deposit(a,ProcessBankDeposit);break;case 1:if(a<MyCharacter.Gold)a=MyCharacter.Bank;vc.ms.Widthdraw(a,ProcessBankWithdraw)}}function ProcessBankDeposit(a,b){ProcessBankTransaction(a,b,0)}
function ProcessBankWithdraw(a,b){ProcessBankTransaction(a,b,1)}
...
function SubmitBankTransfer(){var a=_("transferAmount")[0].value,b=_("transferTarget")[0].value;vc.ms.Transfer(a,b,ProcessBankTransfer)}function BuildExit(a){var b=MyCharacter.CurrentMap.Places[MyCharacter.PositionX][MyCharacter.PositionY];a.append("<h1>"+(b.Name||"Exit")+"</h1>");b=$("<button class='button'>Exit to "+b.ExitTo+"</button>").bind("click",function(d){d.preventDefault();vc.ms.ChangeMap(ProcessMapChange)});b=$("<div class='locationExit'/>").append(b);a.append(b)}
function ProcessMapChange(a,b){if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS){window.MyCharacter.CurrentMap=Maps[a.Data.MapId];window.MyCharacter.PositionX=a.Data.PositionX;window.MyCharacter.PositionY=a.Data.PositionY;BuildMap(true);BuildGameWindow();UpdateStats()}Log("Map Change: "+JSON.stringify(b))}
function BuildBank(a){a.append("<h1>"+(MyCharacter.CurrentMap.Places[MyCharacter.PositionX][MyCharacter.PositionY].Name||"Bank")+"</h1>");var b=$("<form id='bankTransactionForm'></form>"),d=$("<div id='bankTransaction'><h3>You have "+window.MyCharacter.Bank+" gold in your account.</h3></div>"),e=$("<select id='transactionTypeSelection'><option value='0'>Deposit</option><option value='2'>Deposit All</option><option value='1'>Withdraw</option><option value='3'>Withdraw All</option></select>"),f=$("<input type='text' id='transactionAmount' />"),
g=$("<button id='submitTransaction' class='button'>Submit</buy>").bind("click",function(h){h.preventDefault();SubmitBankTransaction()});d.append(e).append(f).append(g);b.append(d);$("<form id='bankTransferForm'></form>");d=$("<div id='bankTransfer'><h3>Transfer</h3></div>");e=$("<div class='bankTransfer transferAmount'><label for='transferAmount'>Amount:</label> <input type='text' id='transferAmount' /></div>");f=$("<div class='bankTransfer transferTarget'><label for='transferTarget'>Send to:</label> <input type='text' id='transferTarget' /></div>");
...
function ReviveCharacter(a,b){if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS){MyCharacter.Health=MyCharacter.Vitality;UpdateStats();BuildGameWindow()}Log("Revive: "+JSON.stringify(b))}
function Attack(a){SetEnableAttack(false);$("#fightResults .result").remove();var b=_("monsterList")[0].value,d=_("fightResults");if(MyCharacter.Health<1)alert("You can't fight! You're dead!");else if(b.indexOf("MONS")>-1){if(MyCharacter.CurrentBattle!==undefined){MyCharacter.CurrentBattle.State==2&&d.html("");if(MyCharacter.CurrentBattle.Enemy==V2Core.Monsters[b])MyCharacter.CurrentBattle.State=1;else MyCharacter.CurrentBattle={Enemy:V2Core.Monsters[b],State:0}}else MyCharacter.CurrentBattle={Enemy:V2Core.Monsters[b],
...
function AttackRound(a,b){window.setTimeout(function(){SetEnableAttack(true)},1500);if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS){var d=_("fightResults"),e=a.Data;$(".round",d).length>0?$(".round",d).animate({opacity:0},250,function(){$(this).remove();BuildAttackMessage(e,MyCharacter.CurrentBattle.Enemy.Name,true,d)}):BuildAttackMessage(e,MyCharacter.CurrentBattle.Enemy.Name,
true,d)}else Log("Attack Error: "+JSON.stringify(b))}
function BuildShop(a){a.append("<h1>"+(MyCharacter.CurrentMap.Places[MyCharacter.PositionX][MyCharacter.PositionY].Name||"Shop")+"</h1>");var b=$("<form id='buyForm'></form>"),d=$("<select id='itemTypeSelection'></select>").bind("change",function(){FilterItemTypes(this.value)}),e=$("<select id='itemSelection'></select>"),f=$("<button id='buyItem' class='button'>Purchase</buy>").bind("click",function(i){i.preventDefault();BuyItem()}),g=$("<button id='itemInfo' class='button'>Info</buy>").bind("click",function(i){i.preventDefault();
i=$("#itemSelection option:selected").prevAll().length;if(i<0)i=0;var o=_("itemTypeSelection")[0].value.split("|");DisplayItemInfo(V2Core.ItemTypes[2][o[0]][o[1]][i])}),h=$("<div id='itemDescription'></div>");e=$("<div><label for='itemTypeSelection'>Shop For:</span></div>").append(d).append(e);var j={};j=$("<optgroup label='Weapons'><option value='0|0'>Swords</option><option value='0|1'>Axes</option><option value='0|2'>Staves</option>option value='0|3'>Maces</option></optgroup>");var k=$("<optgroup label='Armors'><option value='1|0'>Armors</option><option value='1|1'>Shields</option></optgroup>"),
n=$("<optgroup label='Spells'><option value='3|0'>Fire</option><option value='3|1'>Cold</option><option value='3|2'>Air</option><option value='3|3'>Heal</option></optgroup>"),l=$("<optgroup label='Accessories'><option value='2|0'>Accessories</option></optgroup>");d.append(j).append(k).append(n).append(l);b.append(e).append(f).append(g).append(h);a.append(b);b=$("<form id='sellForm'></form>");d=$("<select id='sellSelection'></select>");j={};for(f=0;f<MyCharacter.Inventories.Personal.length;f++){g=
"";if(f==0)g=" selected='selected'";j=MyCharacter.Inventories.Personal[f];d.append("<option value='"+j.ItemId+"'"+g+">"+j.Name+" - "+j.SellPrice+"</option>")}f=$("<button id='sellItem' class='button'>Sell</buy>").bind("click",function(i){i.preventDefault();SellItem()});g=$("<button id='sellInfo' class='button'>Info</buy>").bind("click",function(i){i.preventDefault();i=$("#sellSelection option:selected").prevAll().length;if(i<0)i=0;DisplayItemInfo(MyCharacter.Inventories.Personal[i])});h=$("<div id='sellDescription'></div>");
d=$("<div><label for='sellSelection'>Sell:</span></div>").append(d);b.append(d).append(f).append(g).append(h);a.append(b);FilterItemTypes("0|0")}function SellItem(){var a=_("sellSelection")[0].value;if(a!==null&&a!=""){itemId=a;vc.ms.Sell(itemId,ProcessSale)}}function BuyItem(){MyCharacter.Inventories.Personal.length<20&&vc.ms.Buy(_("itemSelection")[0].value,ProcessPurchase)}
...
a.Name})}function FilterItemTypes(a){var b={},d=_("itemSelection").empty();a=a.split("|");var e=V2Core.ItemTypes[2][a[0]][a[1]];for(item in V2Core.ItemTypes[2][a[0]][a[1]])if(item!="remove"){b=e[item];f="";if(item==0)var f=" selected='selected'";d.append("<option value='"+b.ItemId+"'"+f+">"+b.Name+" - "+b.BuyPrice+"</option>")}}
function ProcessPurchase(a,b){if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS){var d=a.Data;MyCharacter.Inventories.Personal[MyCharacter.Inventories.Personal.length]=d;MyCharacter.Gold-=d.BuyPrice;UpdateStats();BuildInitialInventory();BuildInventoryLists();BuildGameWindow()}Log("Process Purchase: "+JSON.stringify(b))}
function ProcessSale(a,b){var d=b.ItemId;if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS){var e={},f=0,g;for(g in MyCharacter.Inventories.Personal)if(isInteger(g)){if(MyCharacter.Inventories.Personal[g].ItemId==d)e=MyCharacter.Inventories.Personal[g];f++}f--;d=e.SellPrice;MyCharacter.Inventories.Personal.remove(f);MyCharacter.Gold+=d;UpdateStats();BuildInitialInventory();
BuildInventoryLists();BuildGameWindow()}Log("Process Sale: "+JSON.stringify(b))}
function BuildShrine(a){var b=$("<form id='shrineForm'><h1>"+(MyCharacter.CurrentMap.Places[MyCharacter.PositionX][MyCharacter.PositionY].Name||"Shrine")+"</h1></form>");if(MyCharacter.Health>0)b.append("<h2>Why are you here? You're not dead.</h2>");else{var d=$("<button type='submit' id='reviveButton' class='button'>Revive</button>");d.bind("click",function(e){e.preventDefault();vc.ms.Revive(ReviveCharacter)});b.append(d)}$(a).append(b)}
...
function BuildAttackMessage(a,b,d,e){var f=["player","enemy"],g=["attacked","casted","healed"],h=["SMASHED","BLASTED","RECHARGED"],j="";j={};var k=$("<div class='round' />"),n=a.Rounds,l=0;d||(l=1);for(r in n)if(isInteger(r)){var i=n[r];j=i.Actor==l?"You":b;var o="";if(a.Special!==undefined&&a.Special>0)o=" <span class='monsterType "+vc.MonsterSpecialTypes[a.Special]+"'>"+vc.MonsterSpecialTypes[a.Special]+"</span>";if(i.Type==2){if(i.Actor==l){MyCharacter.Health+=i.Damage;if(MyCharacter.Health>MyCharacter.Vitality)MyCharacter.Health=
MyCharacter.Vitality;UpdateHealth()}}else if(i.Actor!=l){MyCharacter.Health-=i.Damage;UpdateHealth()}if(l==1)f=["enemy","player"];var p=o,q=g[i.Type];if(i.IsCritical)q="<strong>"+h[i.Type]+"</strong>";if(i.Actor==l)p="";j=i.Damage>0?$("<p class='result'><span class='attacker "+f[i.Actor]+"'>"+j+"</span>"+p+" "+q+" for <span class='damage'>"+i.Damage+"</span></p>"):$("<p class='result'><span class='attacker "+f[i.Actor]+"'>"+j+"</span>"+p+" <span class='damage'>missed</span></p>");k.append(j)}e.append(k);
if(a.Masteries!==undefined&&a.Masteries.length>0){f={};g=["Armor","Sword","Axe","Mace","Staff","Bow","Fire","Air","Cold","Earth","Shadow","Arcane"];h="";for(n=0;n<a.Masteries.length;n++){h=g[a.Masteries[n]];f=$("<p class='result mastery "+h+"'><span class='attacker player'>Your</span> <strong>"+h+"</strong> mastery has increased!</p>");k.append(f);Log("Mastery gain in "+h)}}if(a.Winner!==undefined){if(a.Winner==l){d?e.append("<div class='result wonFight'><span class='attacker player'>You</span> have defeated <span class='enemy'>"+
b+"</span>"+o+"!</span></div>"):e.append("<div class='result wonFight'><span class='attacker player'>You</span> have successfully defended yourself!</span></div>");if(a.Gold!==undefined)MyCharacter.Gold+=a.Gold;if(a.Experience!==undefined)MyCharacter.Experience+=a.Experience;if(a.AlignGood!==undefined)MyCharacter.AlignGood=a.AlignGood;if(a.AlignOrder!==undefined)MyCharacter.AlignOrder=a.AlignOrder;if(a.LevelUp!==undefined&&a.LevelUp==true){e.append("<div class='result levelUp'><span class='attacker player'>You</span> have leveled up! <a href='#' class='chooseStats button'>Choose Stats</a></span></div>");
MyCharacter.Experience-=MyCharacter.NextLevelAt();MyCharacter.FreeLevels++}}else{e.append("<div class='result lostFight'><span class='attacker enemy'>You</span> were defeated!</span></div>");MyCharacter.Health=0;MyCharacter.Gold=0}UpdateStats()}return e}
function DisplayChannelStatusUpdate(a,b){var d=$("input[value='"+a.ChannelId+"']").parent();if(window.MyCharacter.Channels[a.ChannelId]===undefined){d=a.Name;if(d.indexOf("!!PM")==0)d="a private channel";$("<div class='chatMessage system'>"+b.FromName+" invited you to join <span class='channelName'>"+d+"<input type='hidden' value='"+a.Name+"' /></span>! <a href='#' class='joinChannel'>Click here to join.</a></div>").dialog({title:"Chat Room Invitation"})}else if(window.MyCharacter.Channels[a.ChannelId].Permissions.isJoined==
1&&a.isJoined==0){LeaveChannel(a.ChannelId);$("<div class='chatMessage system'>You have been kicked from <span class='channelName'>"+a.Name+"</span>!</div>").dialog({title:"Kicked From Chat Room"})}else{window.MyCharacter.Channels[a.ChannelId].Permissions.Write==1&&a.Write==0&&$("<div class='chatMessage system'>You have been muted in this channel.</div>").prependTo(d);if(window.MyCharacter.Channels[a.ChannelId].Permissions.Moderate==1&&a.Moderate==0)$("<div class='chatMessage system'>You are no longer a mod in this channel.</div>").prependTo(d);
...
vc.ch.SetParameters(d,"PublicWrite",a,function(e,f){ProcessChannelParamterChange(d,f.Parameter,f[0].value)})}}else{vc.ch.SendMessageToChannel(MyCharacter.CurrentChannel,a,function(){});InsertChat([{Type:b.Type,FromName:MyCharacter.Name,Message:b.Message,AlignGood:MyCharacter.AlignGood,AlignOrder:MyCharacter.AlignOrder}],d)}}}function ProcessChannelParamterChange(a,b,d){b=="Motd"&&InsertChat([{Type:999,FromName:"",Message:d}],a)}
...
_("myCharacter_Alignment").text(b+" ("+a.AlignGood+" / "+a.AlignOrder+")").siblings(".statLabel").addClass(d);MyCharacter.FreeLevels>0&&$("#statsWindow button, #statsWindow .all").show()}function UpdateHealth(){var a=window.MyCharacter;ICache.myCharacter_HealthBar.progressbar("value",a.Health/a.Vitality*100)[0].setAttribute("title",a.Health+" / "+a.Vitality);ICache.myCharacter_Health.text(a.Health)}
