...
function Attack(a){SetEnableAttack(false);$("#fightResults .result").remove();var b=_("monsterList")[0].value,d=_("fightResults");if(window.MyCharacter.Health<1)alert("You can't fight! You're dead!");else if(b.indexOf("MONS")>-1){if(window.MyCharacter.CurrentBattle!==undefined){window.MyCharacter.CurrentBattle.State==2&&d.html("");if(window.MyCharacter.CurrentBattle.Enemy==V2Core.Monsters[b])window.MyCharacter.CurrentBattle.State=1;else window.MyCharacter.CurrentBattle={Enemy:V2Core.Monsters[b],State:0}}else window.MyCharacter.CurrentBattle=
{Enemy:V2Core.Monsters[b],State:0};window.MyCharacter.CurrentBattle={Enemy:V2Core.Monsters[b],State:0};vc.mn.Fight(b,a,AttackRound)}else if(b.indexOf("CHAR")>-1){window.MyCharacter.CurrentBattle={Enemy:{Id:b,Name:$("#monsterList :selected").text()},State:0};vc.cs.Fight(b,a,AttackRound)}}
...
function(i){i.preventDefault();i=$("#itemSelection option:selected").prevAll().length;if(i<0)i=0;var o=_("itemTypeSelection")[0].value.split("|");DisplayItemInfo(V2Core.ItemTypes[2][o[0]][o[1]][i])}),h=$("<div id='itemDescription'></div>");e=$("<div><label for='itemTypeSelection'>Shop For:</span></div>").append(d).append(e);var j={};j=$("<optgroup label='Weapons'><option value='0|0'>Swords</option><option value='0|1'>Axes</option><option value='0|2'>Staves</option>option value='0|3'>Maces</option></optgroup>");
var k=$("<optgroup label='Armors'><option value='1|0'>Armors</option><option value='1|1'>Shields</option></optgroup>"),n=$("<optgroup label='Spells'><option value='3|0'>Fire</option><option value='3|1'>Cold</option><option value='3|2'>Air</option><option value='3|3'>Heal</option></optgroup>"),l=$("<optgroup label='Accessories'><option value='2|0'>Accessories</option></optgroup>");d.append(j).append(k).append(n).append(l);b.append(e).append(f).append(g).append(h);a.append(b);b=$("<form id='sellForm'></form>");
d=$("<select id='sellSelection'></select>");j={};for(f=0;f<window.MyCharacter.Inventories.Personal.length;f++){g="";if(f==0)g=" selected='selected'";j=window.MyCharacter.Inventories.Personal[f];d.append("<option value='"+j.ItemId+"'"+g+">"+j.Name+" - "+j.SellPrice+"</option>")}f=$("<button id='sellItem' class='button'>Sell</buy>").bind("click",function(i){i.preventDefault();SellItem()});g=$("<button id='sellInfo' class='button'>Info</buy>").bind("click",function(i){i.preventDefault();i=$("#sellSelection option:selected").prevAll().length;
if(i<0)i=0;DisplayItemInfo(window.MyCharacter.Inventories.Personal[i])});h=$("<div id='sellDescription'></div>");d=$("<div><label for='sellSelection'>Sell:</span></div>").append(d);b.append(d).append(f).append(g).append(h);a.append(b);FilterItemTypes("0|0")}function SellItem(){var a=_("sellSelection")[0].value;if(a!==null&&a!=""){itemId=a;vc.ms.Sell(itemId,ProcessSale)}}function BuyItem(){window.MyCharacter.Inventories.Personal.length<20&&vc.ms.Buy(_("itemSelection")[0].value,ProcessPurchase)}
...
function ProcessSale(a,b){var d=b.ItemId;if(vc.DebugMode&&a.RequestDuration>0){vc.Requests++;vc.RequestDurationTotal+=a.RequestDuration;ICache.rda_value.text(vc.RequestDurationTotal/vc.Requests)}if(a.Result==vc.ER_SUCCESS){var e={},f=0,g;for(g in window.MyCharacter.Inventories.Personal)if(isInteger(g)){if(window.MyCharacter.Inventories.Personal[g].ItemId==d)e=window.MyCharacter.Inventories.Personal[g];f++}f--;d=e.SellPrice;window.MyCharacter.Inventories.Personal.remove(f);window.MyCharacter.Gold+=
d;UpdateStats();BuildInitialInventory();BuildInventoryLists();BuildGameWindow()}Log("Process Sale: "+JSON.stringify(b))}
...
function BuildAttackMessage(a,b,d,e){var f=["player","enemy"],g=["attacked","casted","healed"],h=["SMASHED","BLASTED","RECHARGED"],j="";j={};var k=$("<div class='round' />"),n=a.Rounds,l=0;d||(l=1);for(r in n)if(isInteger(r)){var i=n[r];j=i.Actor==l?"You":b;var o="";if(a.Special!==undefined&&a.Special>0)o=" <span class='monsterType "+vc.MonsterSpecialTypes[a.Special]+"'>"+vc.MonsterSpecialTypes[a.Special]+"</span>";if(i.Type==2){if(i.Actor==l){window.MyCharacter.Health+=i.Damage;if(window.MyCharacter.Health>
window.MyCharacter.Vitality)window.MyCharacter.Health=window.MyCharacter.Vitality;UpdateHealth()}}else if(i.Actor!=l){window.MyCharacter.Health-=i.Damage;UpdateHealth()}if(l==1)f=["enemy","player"];var p=o,q=g[i.Type];if(i.IsCritical)q="<strong>"+h[i.Type]+"</strong>";if(i.Actor==l)p="";j=i.Damage>0?$("<p class='result'><span class='attacker "+f[i.Actor]+"'>"+j+"</span>"+p+" "+q+" for <span class='damage'>"+i.Damage+"</span></p>"):$("<p class='result'><span class='attacker "+f[i.Actor]+"'>"+j+"</span>"+
p+" <span class='damage'>missed</span></p>");k.append(j)}e.append(k);if(a.Masteries!==undefined&&a.Masteries.length>0){f={};g=["Armor","Sword","Axe","Mace","Staff","Bow","Fire","Air","Cold","Earth","Shadow","Arcane"];h="";for(n=0;n<a.Masteries.length;n++){h=g[a.Masteries[n]];f=$("<p class='result mastery "+h+"'><span class='attacker player'>Your</span> <strong>"+h+"</strong> mastery has increased!</p>");k.append(f);Log("Mastery gain in "+h)}}if(a.Winner!==undefined){if(a.Winner==l){d?e.append("<div class='result wonFight'><span class='attacker player'>You</span> have defeated <span class='enemy'>"+
b+"</span>"+o+"!</span></div>"):e.append("<div class='result wonFight'><span class='attacker player'>You</span> have successfully defended yourself!</span></div>");if(a.Gold!==undefined)window.MyCharacter.Gold+=a.Gold;if(a.Experience!==undefined)window.MyCharacter.Experience+=a.Experience;if(a.AlignGood!==undefined)window.MyCharacter.AlignGood=a.AlignGood;if(a.AlignOrder!==undefined)window.MyCharacter.AlignOrder=a.AlignOrder;if(a.LevelUp!==undefined&&a.LevelUp==true){e.append("<div class='result levelUp'><span class='attacker player'>You</span> have leveled up! <a href='#' class='chooseStats button'>Choose Stats</a></span></div>");
window.MyCharacter.Experience-=window.MyCharacter.NextLevelAt();window.MyCharacter.FreeLevels++}}else{e.append("<div class='result lostFight'><span class='attacker enemy'>You</span> were defeated!</span></div>");window.MyCharacter.Health=0;window.MyCharacter.Gold=0}UpdateStats()}return e}
