$(function(){FB.init({appId:"119442588120693",status:true,cookie:true,xfbml:true});$("#playNow").dialog({modal:true,title:"Log In / Register a New Account",width:600,autoOpen:false});$("#btnPlayNow, .playNow").bind("click",function(a){a.preventDefault();if($.cookie("l")=="true")window.location="./account.php";else $("#playNow").dialog("open")});$("#registerForm").bind("submit",function(a){a.preventDefault();a=true;var b=$("#ca_username").val(),c=$("#ca_password").val(),d=$("#ca_email").val();if(b.length<
4){a=false;$("#ca_username_validator").html("<span class='ui-state-error ui-corner-all'><span class='ui-icon ui-icon-alert'></span>Username too short.</span>")}if(c.length<4){a=false;$("#ca_password_validator").html("<span class='ui-state-error ui-corner-all'><span class='ui-icon ui-icon-alert'></span>Password too short.</span>")}a&&RegisterAccount(b,c,d);return false});$("#loginForm").bind("submit",function(a){a.preventDefault();if($("ui-state-error:visible",$(".logIn")).length>0)return false;a=
$("#li_username").val();var b=$("#li_password").val();LogInAccount(a,b);return false});$("#quickLoginForm").bind("submit",function(a){a.preventDefault();if($("ui-state-error:visible",$("#topLoginForm")).length>0)return false;a=$("#quickLogin_un").val();var b=$("#quickLogin_pw").val();LogInAccount(a,b);return false});$("#fbregister").bind("click",function(a){a.preventDefault();var b="",c="",d="";FB.getLoginStatus(function(e){if(e.session){b=e.session.uid;c=e.session.access_token;FB.api("/me",function(f){d=
f.email;RegisterAccount(b,c,d)})}else FB.login(function(f){if(f.session){b=f.session.uid;c=f.session.access_token;f.perms&&FB.api("/me",function(g){d=g.email;RegisterAccount(b,c,d)})}},{perms:"read_stream,publish_stream,offline_access,email,create_event,user_birthday"})})});$("#fblogin").bind("click",function(a){a.preventDefault();var b="",c="";FB.getLoginStatus(function(d){if(d.session){b=d.session.uid;c=d.session.access_token;LogInAccount(b,c)}else FB.login(function(e){if(e.session){b=e.session.uid;
c=e.session.access_token;LogInAccount(b,c)}})})});vc.api.Online(ProcessOnline);vc.api.GetTops(30,0,0,0,"",ProcessTops)});function ProcessOnline(a){a.Result==vc.ER_SUCCESS&&$("#onlines").text(a.Data.Count).removeClass("loading")}
function ProcessTops(a){if(a.Result==vc.ER_SUCCESS){for(var b={},c="",d=0;d<a.Data.length;d++){b=a.Data[d];c=vc.Races[b.RaceId].Name;$("#topList").append("<li title='#"+(d*1+1)+" | "+b.Name+" | "+AlignName(b.AlignGood,b.AlignOrder)+" Level "+b.Level+" "+c+"' class='"+c+"'></li>")}$("#topList li").tipsy({gravity:$.fn.tipsy.S,fade:true,html:true})}}function LogInAccount(a,b){vc.as.Login(a,$.md5(b),function(c){ProcessLogin(c.Result)})}
