...
class Chat extends \Database\Chat
{
	 ...
	 */
	public function Insert(\Entities\Character $Character, $Channel, $Message)
	{
		$Query = $this->Database->Connection->prepare(SQL_INSERTMESSAGE);
		$Query->bind_param('ssss', $Character->CharacterId, $Channel, $Message, $Character->Name);
	 ...
	 */
	public function LoadListForChannel(\Entities\Character $Character, $Channel, $DateForward)
	{
		$Query = $this->Database->Connection->prepare(SQL_GETMESSAGES);
		$Query->bind_param('sss', $Character->CharacterId, $Channel, $DateForward);
	 ...
	 */
	public function JoinChannel(\Entities\Character $Character, $Channel)
	{
		$Query = $this->Database->Connection->prepare(SQL_JOINCHANNEL);
		$Query->bind_param('ss', $Character->CharacterId, $Channel);
	 ...
	 */
	public function GetRights(\Entities\Character $Character, $Channel)
	{
		$Query = $this->Database->Connection->prepare(SQL_JOINCHANNEL);
		$Query->bind_param('ss', $Character->CharacterId, $Channel);
